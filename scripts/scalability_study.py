        ...
        for dataset in datasets:
            for executable in executables:
                for model in models:
                    ...
                    #model= model+'_'+str(bsize)+'.engine'
                        #l= str(warmup)+'_effect.log'
                        os.system('./build/' +executable+ loc_trace +dataset+ '.qq100m.tr.bin'+loc_aux +dataset+ '.qq100m.tra.bin ~/final_libtorch_models/' +model+instructions+' ' + str(bsize)+' ' + str(GPU) +str(warmup))
